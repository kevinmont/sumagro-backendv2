...
import { SackDao } from '../sackDao';

const logger = log4js.getLogger('sumagro.dao.impl.SackDaoImpl');

export class SackDaoImpl extends AbstractDao implements SackDao{
    ...
    async getIds(ingenioId: number, table: string, producto: string, page: number, peer_page: number){
        ...
        let sql="";
        if(ingenioId!=-1){
        sql= `select id from ${table}  WHERE ingenioid=${ingenioId} and description='${producto}' LIMIT ${page},${peer_page};`;
        }else{
            sql= `select id from ${table}  WHERE  description='${producto}' LIMIT ${page},${peer_page};`;
        }
