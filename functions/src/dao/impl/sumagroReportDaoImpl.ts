...
import { SumagroReportDao } from '../sumagroReportDao';

const logger = log4js.getLogger('sumagro.dao.impl.SumagroReportDaoImpl');

export class SumagroReportDaoImpl extends AbstractDao implements SumagroReportDao{
    ...
    async getAplicatedOutParcel(dateStart:string,dateEnd:string){
       ...
       if(productores.length){
        ...
        let parcelas:any  = await this.db.query(queryOfParcel);
